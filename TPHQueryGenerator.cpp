...
TPHQueryGenerator::TPHQueryGenerator(std::vector<int> queryIDs){
    ...
    statistics = {
	    {"Q5_TOTAL", millisecond::zero()},
	    {"Q8_TOTAL", millisecond::zero()},
	    {"Q20_TOTAL", millisecond::zero()},
	    {"NUM_RUNS", millisecond::zero()},
...
bool TPHQueryGenerator::runQueries(unsigned int runs){
    for (unsigned int run = 1; run <= runs; run++){
	    ...
	    switch (i){
		    case 0:
			    statistics["Q5_TOTAL"] += duration;
			    break;
		    case 1:
			    statistics["Q8_TOTAL"] += duration;
			    break;
		    case 2:
			    statistics["Q20_TOTAL"] += duration;
			    break;
		    default:
			    break;
	    }
